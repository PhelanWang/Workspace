...
# 用得到的模型估计已有的分类样本
results = np.zeros((100, 1), dtype=np.int)
for i in range(0, Y.size):
    X0_Y0 = normal_distribution(X[i, 0], X_Y0[:, 0].mean(), X_Y0[:, 0].var())
    X1_Y0 = normal_distribution(X[i, 1], X_Y0[:, 1].mean(), X_Y0[:, 1].var())
    result0 = P_Y0 * (X0_Y0 + X1_Y0)
    X0_Y1 = normal_distribution(X[i, 0], X_Y1[:, 0].mean(), X_Y1[:, 0].var())
    X1_Y1 = normal_distribution(X[i, 0], X_Y1[:, 1].mean(), X_Y1[:, 1].var())
    result1 = P_Y1 * (X0_Y1 + X1_Y1)
    results[i] = result1 > result0

# 计算用得到的模型得出的分类的正确率
print '用得到的模型得出的分类正确率为: %.2f%%' % (float(sum(results == Y))/Y.size)